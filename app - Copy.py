...
def parse_and_sort_logic(markdown_text):
    """Deterministic Python parser for sorting and formatting."""
    ...
    # STRICT DESCENDING SORT by Date
...
if st.session_state.get('trigger_fetch'):
    ...
    with st.spinner("🧠 Scanning for Payments, Orders, and Winnings..."):
        # Gmail: Scanning for Finance Keywords
        ...
        for m in messages:
            ...
            # Pre-Processor: Flagging critical financial keywords
        ...
        4. DATE: Use YYYY-MM-DD.
        5. FORMAT: No parentheses.
...
if user_query and st.session_state.full_context:
    model = genai.GenerativeModel('gemini-2.5-flash')
    chat_prompt = f"""
    You are a Personal Assistant. Focus on upcoming payments, Temu orders, and winnings.
